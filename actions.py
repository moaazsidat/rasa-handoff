...
from rasa_core.events import SlotSet, AllSlotsReset, ConversationPaused, ConversationResumed
import requests
import json
...
class ActionMeetup(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		response = "passing info to human – location: {}, type: {}".format(location, type)
		dispatcher.utter_message(response)
...
class ActionJoinEvent(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		try:	
			...
			dispatcher.utter_message("Sorry, there are no free seats left in this event.")
